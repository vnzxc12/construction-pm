...
export default function DrawingsPage({ params }: { params: { id: string } }) {
  ...
  const handleUploadSubmit = async (e: React.FormEvent) => {
    ...
    const newDocPayload = {
      ...
      title: docTitle,
      sheet_number: sheetNumber || "A-101",
      category: docCategory,
      version: 1,
      file_url: "https://example.com/mock-blueprint.pdf",
      file_size_bytes: 14500000,
      storage_path: `blueprints/${params.id}/${sheetNumber || 'sheet'}.pdf`,
      description: "Uploaded drawing revision to Supabase Storage.",
      uploaded_by: user?.id,
    };
    ...
    <div className="space-y-6">
      ...
      <div className="bg-slate-900 text-slate-200 p-4 rounded-xl border border-slate-800 flex items-center justify-between text-xs">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <span className="text-slate-400">Postgres metadata &bull; Global CDN delivery</span>
          </div>
        </div>
      ...
      {loading ? (
        <div className="py-20 flex flex-col items-center justify-center text-slate-500 space-y-3">
          ...
          <span className="text-sm font-medium">Loading Blueprints from Supabase...</span>
        </div>
      ) : filteredDocs.length === 0 ? (
        <div className="bg-white rounded-2xl border border-dashed border-slate-300 p-12 text-center max-w-lg mx-auto">
          <FileSpreadsheet className="w-10 h-10 text-slate-400 mx-auto mb-2" />
          <h3 className="text-base font-bold text-slate-900">No Blueprints Uploaded Yet</h3>
          <p className="text-xs text-slate-500 mt-1 mb-6">
            Upload architectural, structural, or MEP drawing sets to your Supabase Storage bucket.
          </p>
          <button
            type="button"
            onClick={() => setShowUploadModal(true)}
        ...
        <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl max-w-lg w-full p-6 shadow-2xl border border-slate-200">
            ...
            <p className="text-xs text-slate-500 mt-1">
              File will be uploaded to Supabase Storage for {project?.name || "Project"}
            </p>
            ...
            <form onSubmit={handleUploadSubmit} className="space-y-4 mt-5">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Floor Plan Level 18"
