    ...
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-dashed border-slate-300 dark:border-slate-800 p-12 text-center max-w-lg mx-auto">
          ...
          <h3 className="text-base font-bold text-slate-900 dark:text-white">No Daily Reports Yet</h3>
        ...
        <div className="space-y-6">
          {logs.map((log) => {
            ...
            return (
              <div
                key={log.id}
                className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm p-6 space-y-5 transition-colors"
              >
                {/* Log Header */}
                <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-4 border-b border-slate-100 dark:border-slate-800 gap-3">
                  <div className="flex items-center gap-3">
                    <div className="w-12 h-12 rounded-xl bg-amber-500/10 dark:bg-amber-500/20 text-amber-600 dark:text-amber-400 flex flex-col items-center justify-center font-bold border border-amber-200/40 dark:border-amber-500/30">
                      <span className="text-[10px] uppercase tracking-wider text-amber-700 dark:text-amber-300">Date</span>
                    ...
                    <div>
                      <h3 className="font-bold text-slate-900 dark:text-white text-base flex items-center gap-2">
                        ...
                        <span className="text-xs font-mono bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700 px-2 py-0.5 rounded">
                          {log.log_date}
                        </span>
                      </h3>
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    <div className="inline-flex items-center gap-1.5 bg-sky-50 dark:bg-sky-950/40 text-sky-800 dark:text-sky-300 border border-sky-200 dark:border-sky-800 px-3 py-1 rounded-lg text-xs font-medium">
                      <CloudSun className="w-4 h-4 text-sky-600 dark:text-sky-400" />
                      <span>{log.weather_condition || "Clear"}</span>
                    </div>
                    {totalCrew > 0 && (
                      <div className="inline-flex items-center gap-1.5 bg-emerald-50 dark:bg-emerald-950/40 text-emerald-800 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800 px-3 py-1 rounded-lg text-xs font-semibold">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-800 dark:text-slate-100 bg-slate-50 dark:bg-slate-800/70 p-4 rounded-xl border border-slate-200/70 dark:border-slate-700/80 leading-relaxed font-medium">
                    {log.work_performed}
                  </p>
                </div>
                      ...
                      <table className="w-full text-left text-xs">
                        ...
                        <tbody className="divide-y divide-slate-100 dark:divide-slate-800 text-slate-700 dark:text-slate-300">
                          {log.daily_log_crews.map((crew) => (
                            <tr key={crew.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/50">
                              <td className="px-4 py-2 font-semibold text-slate-900 dark:text-white">{crew.contractor_name}</td>
                              <td className="px-4 py-2 text-slate-600 dark:text-slate-300">{crew.trade}</td>
                              <td className="px-4 py-2 text-center font-bold text-amber-600 dark:text-amber-400 font-mono">{crew.worker_count} Workers</td>
                              <td className="px-4 py-2 text-right font-mono">{crew.hours_worked} hrs</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
                  <div className="p-3.5 bg-emerald-50/50 dark:bg-emerald-950/30 rounded-xl border border-emerald-200/60 dark:border-emerald-900/50">
                    <div className="flex items-center gap-2 text-emerald-800 dark:text-emerald-300 font-bold text-xs mb-1">
                      <ShieldCheck className="w-4 h-4 text-emerald-600 dark:text-emerald-400" />
                      <span>Safety Compliance</span>
                    </div>
                    <p className="text-xs text-slate-700 dark:text-slate-300">{log.safety_incidents || "Zero incidents."}</p>
                  </div>

                  <div className="p-3.5 bg-amber-50/50 dark:bg-amber-950/30 rounded-xl border border-amber-200/60 dark:border-amber-900/50">
                    ...
                    <p className="text-xs text-slate-700 dark:text-slate-300">{log.delays_notes || "No delays recorded."}</p>
                  </div>
                </div>
              </div>
            );
        ...
        <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white dark:bg-slate-900 rounded-2xl max-w-2xl w-full p-6 shadow-2xl border border-slate-200 dark:border-slate-800 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
              <div>
                <h2 className="text-xl font-bold text-slate-900 dark:text-white">Record Daily Field Report</h2>
            ...
            <form onSubmit={handleCreateLog} className="space-y-4 mt-4">
              ...
              <div className="bg-slate-50 dark:bg-slate-800/70 p-4 rounded-xl border border-slate-200 dark:border-slate-700 space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-1.5 font-bold text-xs text-slate-800 dark:text-slate-200 uppercase tracking-wider">
                ...
                <div className="space-y-2">
                  {crewInputs.map((crew, idx) => (
                    <div key={idx} className="grid grid-cols-12 gap-2 items-center bg-white dark:bg-slate-900 p-2.5 rounded-lg border border-slate-200 dark:border-slate-700 text-xs">
                      <div className="col-span-4">
                        <input
                          ...
                          className="w-full px-2.5 py-1.5 border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500 rounded focus:outline-none focus:ring-1 focus:ring-amber-500"
                        />
                      </div>
                      <div className="col-span-4">
                        <input
                      ...
                      <div className="col-span-2">
                        <input
                          ...
                          placeholder="Count"
                          className="w-full px-2 py-1.5 border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white font-bold text-center rounded focus:outline-none focus:ring-1 focus:ring-amber-500"
                        />
                      </div>
                      <div className="col-span-2 flex items-center justify-end gap-1">
                        <span className="text-[10px] text-slate-400 dark:text-slate-500 font-mono">8h</span>
                        {crewInputs.length > 1 && (
                          <button
                            type="button"
