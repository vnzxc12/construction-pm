-- ==============================================================================
-- FIX FOR: "Storage Error: new row violates row-level security policy"
-- RUN THIS ENTIRE SCRIPT IN YOUR SUPABASE SQL EDITOR (Dashboard -> SQL Editor -> New Query -> Run)
-- ==============================================================================

-- 1. Ensure Storage Extension and Buckets exist and are public
INSERT INTO storage.buckets (id, name, public, file_size_limit, allowed_mime_types)
VALUES 
  ('blueprints', 'blueprints', true, 104857600, NULL),
  ('project-documents', 'project-documents', true, 104857600, NULL),
  ('site-photos', 'site-photos', true, 52428800, NULL),
  ('punch-photos', 'punch-photos', true, 52428800, NULL)
...
-- 3. Create full CRUD policies for storage.objects
-- Note: 'FOR UPDATE' policy is REQUIRED when upload uses { upsert: true }

-- A. SELECT / READ (Public & Authenticated can view plans & photos)
...
-- B. INSERT / UPLOAD (Authenticated & Anon can upload)
...
-- D. DELETE (Allows removing plans / blueprints)
...
-- Create permissive policy for drawings_documents
...
GRANT ALL ON storage.objects TO authenticated, anon, service_role, postgres;
GRANT ALL ON storage.buckets TO authenticated, anon, service_role, postgres;

-- Output confirmation
SELECT 'Storage and Drawings RLS successfully configured!' AS status;
